...
export default function Skills() {
  return (
    <section id="skills" className="py-32 px-6 bg-[#F5F5F5] ambient-orange-glow scroll-mt-20">
      <div className="max-w-6xl mx-auto">

        <div>
          <p className="text-xs tracking-[0.2em] uppercase text-orange-500 mb-3">Expertise</p>
          <h2 className="font-['Lilita_One',sans-serif] text-4xl md:text-5xl font-normal text-[#1A1A1D]">Skills</h2>
          ...
          {categories.map((cat) => (
            <div key={cat.title} className="flex flex-col gap-4">
              ...
              <div className="flex items-center justify-between mb-2 pb-1 border-b border-black/10">
                <h3 className="text-xs font-bold tracking-widest uppercase text-[#1A1A1D]/60">
                  {cat.title}
                </h3>
              ...
              <div className="flex flex-col gap-3">
                {cat.skills.map((skill) => (
                  <div
                    key={skill.name}
                    className="group flex items-center gap-3.5 p-3.5 bg-[#FFFFFF] border border-black/5 rounded-xl hover:border-orange-500/40 hover:shadow-[0_8px_30px_rgba(255,106,28,0.06)] hover:translate-y-[-2px] transition-all duration-300 cursor-default shadow-sm"
                  >
                    {/* Logo Wrapper */}
                    <div className="flex-shrink-0 w-11 h-11 flex items-center justify-center bg-[#F5F5F5] border border-black/5 rounded-lg transition-all duration-300 group-hover:bg-orange-500/5 group-hover:scale-105">
                      {skill.icon}
                    </div>
                    <div>
                      {/* Tech Name */}
                      <h4 className="text-sm font-semibold text-[#1A1A1D] group-hover:text-orange-500 transition-colors leading-none">
                        {skill.name}
                      </h4>
                    </div>
                  </div>
